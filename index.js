...
async function milimnava() {
    ...
    const milimnava = makeWASocket({
        ...
        cachedGroupMetadata: async (jid) => {
            ...
            if (!gm) {
                try {
                    gm = await milimnava.groupMetadata(jid)
    ...
    const _sendMessage = milimnava.sendMessage
    milimnava.sendMessage = async (jid, content, options = {}) => {
    ...
    if (!milimnava.authState.creds.registered) {
        const inputNumber = await question('Masukin nomor yang mau dijadikan bot.. contoh: 62895345377678\n');
        const pairingmilimnava = "MILIMNAV";
        let code = await milimnava.requestPairingCode(inputNumber || phoneNumber, pairingmilimnava);
        code = code?.match(/.{1,4}/g)?.join("-") || code;
        console.log(`Ini kodenya:`, code);
    }
    
    store.bind(milimnava.ev)

    milimnava.ev.on('connection.update', async (update) => {
        const { connection, lastDisconnect } = update
        try {
            if (connection === 'close') {
                let reason = new Boom(lastDisconnect?.error)?.output.statusCode
                if (reason === DisconnectReason.badSession) {
                    console.log(`error woii, coba ulang dengan hapus folder milimnavanya👌`);
                    milimnavaInd()
                    ...
                    console.log(`Sesi terputus.. Mohon hapus folder milimnava`);
                    milimnavaInd();
            ...
            if (update.connection == "open") {
                console.log(`✅ Milimnava Connected!`)
            }
        } catch (err) {
    ...
    global.milimnava = milimnava
    milimnava.ev.on('creds.update', await saveCreds)

    // Anti Call
    milimnava.ev.on('call', async (callUpdate) => {
        try {
            let botNumber = await milimnava.decodeJid(milimnava.user.id)
            ...
            for (let call of calls) {
                ...
                if (call.status === 'offer' || call.status === 'ringing') {
                    try { if (milimnava.rejectCall) await milimnava.rejectCall(call.id, call.from) } catch (e) {}
                    let msg = await milimnava.sendTextWithMentions(
                        call.from,
                        `*${milimnava.user.name}* tidak bisa menerima panggilan ${call.isVideo ? 'video' : 'suara'}.\nMaaf @${call.from.split('@')[0]} kamu akan diblokir.`
                    )
                    await delay(3000)
                    await milimnava.updateBlockStatus(call.from, "block")
                }
            }
    ...
    milimnava.ev.on('messages.upsert', async chatUpdate => {
        try {
            const kay = chatUpdate.messages[0]
            if (!kay.message) return
            kay.message = (Object.keys(kay.message)[0] === 'ephemeralMessage') ? kay.message.ephemeralMessage.message : kay.message
            if (kay.key && kay.key.remoteJid === 'status@broadcast') {
                await milimnava.readMessages([kay.key]) 
            }
            await normalizeMessageIds(milimnava, kay)
            const sender = kay.key.participant || kay.key.remoteJid

            const Yue = [...owner, global.ownernomer, global.botnumber].map(v => v ? v.replace(/[^0-9]/g, '') + '@s.whatsapp.net' : '').includes(sender)

            if (!milimnava.public && !kay.key.fromMe && !Yue && chatUpdate.type === 'notify') return
            if (kay.key.id.startsWith('903D') && kay.key.id.length === 14) return
            
            const m = kay 
            require('./milimnava')(milimnava, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    milimnava.sendTextWithMentions = async (jid, text, quoted, options = {}) => milimnava.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net') }, ...options }, { quoted })

    milimnava.getName = (jid, withoutContact = false) => {
        let id = milimnava.decodeJid(jid)
        withoutContact = milimnava.withoutContact || withoutContact 
        let v
        if (id.endsWith("@g.us")) return new Promise(async (resolve) => {
            v = store.contacts[id] || {}
            if (!(v.name || v.subject)) v = milimnava.groupMetadata(id) || {}
            resolve(v.name || v.subject || PhoneNumber('+' + id.replace('@s.whatsapp.net', '')).getNumber('international'))
        })
        else v = id === '0@s.whatsapp.net' ? { id, name: 'WhatsApp' } : id === milimnava.decodeJid(milimnava.user.id) ? milimnava.user : (store.contacts[id] || {})
        return (withoutContact ? '' : v.name) || v.subject || v.verifiedName || PhoneNumber('+' + jid.replace('@s.whatsapp.net', '')).getNumber('international')
    }

    milimnava.public = true

    milimnava.downloadMediaMessage = async (message) => {
